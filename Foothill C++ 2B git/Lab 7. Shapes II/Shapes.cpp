...
bool Point::draw(Screen& scr, char ch) const {
    // TODO - Complete this method
    /* Type your code here */
...
bool Line::draw(Screen& scr, char ch) const {
    // TODO: Complete this method
    /* Type your code here */
    if (x1 == x2 && y1 == y2) {
...
const std::vector<Shape *>& StickMan::getParts() const {
    return parts;
}


bool Quadrilateral::draw(Screen& scr, char ch) const {
    bool status = true;
    status = Line(x1, y1, x2, y2).draw(scr, ch) && status;
    status = Line(x2, y2, x3, y3).draw(scr, ch) && status;
    status = Line(x3, y3, x4, y4).draw(scr, ch) && status;
    status = Line(x4, y4, x1, y1).draw(scr, ch) && status;
    return status;
}

bool StickMan::draw(Screen& scr, char ch) const {
    bool status = true;
    for (Shape* ptr : parts) {
        status = ptr->draw(scr, ch) && status;
    }
    return status;
}
